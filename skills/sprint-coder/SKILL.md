...
> **背景**: SPRINT-020/021で導入。Wave並列実行時にsprint-coderがサブエージェント（Task tool経由）として起動される場合の追加制約。

### サブエージェントとして起動される条件

sprint-masterが並列モード（Waveベース）でタスクを実行する場合、各タスクはTask toolのサブエージェントとして起動される。この場合、sprint-coderは**独立したコンテキスト**で動作し、メインエージェントとの直接通信はできない。
