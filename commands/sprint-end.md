...
終了してよいですか？未完了タスクは tasks.md に持ち越しとして記録します。
```
...
4. フィードバック処理
   ...
   - 持越し: 上記に該当しない → tasks.md に次スプリント向けタスクとして登録
...
2. **KPT履歴更新**: `{project_root}/kpt-history.md` に追記

3. **メンバー状態更新**: `{project_root}/team-roster.md` の稼働履歴を更新

4. **tasks.md の集計更新**: YAMLフロントマターの集計値を再計算

5. **milestones.md の進捗更新**: 対象マイルストーンの進捗率を再計算
