...
- `{project_root}/scrum/milestones.md` と `{project_root}/scrum/tasks.md` が存在すること
...
- ❌ POの「cp」指示があっても、レビュー/レトロ未完了ならレビュー/レトロを経ずにコミット&pushすること

### cpコマンドガード

> **背景**: SPRINT-022でPOの「cp」指示にライフサイクルを飛ばして従ってしまう問題が発生。

POが「cp」（コミット&push）と指示した場合、sprint-backlog.md のステータスを確認する:

| sprint-backlog.md のステータス | 行動 |
|------|------|
| `in_progress`（レビュー/レトロ未実施） | 「先にレビューとレトロスペクティブを実施します」と案内し、レビューに遷移 |
| `completed`（レビュー/レトロ実施済み） | コミット&push を実行 |

**ライフサイクルの順序はPOの短縮指示に優先する。**
